...
- Supports multiple recurrence patterns:
  - **Daily**: every day (advances +1 day)
  - **Weekdays**: every weekday (business days only - Mon-Fri)
    - Sun-Thu → next calendar day (+1 day)
    - Fri-Sat → following Monday (+3 or 2 days)
  - **Weekly**: every week, every N weeks (advances +7 days or +N weeks)
  - **Monthly**: every month, every N months (advances to same day next month)
  - **Specific weekdays**: every Monday, every Tuesday, etc. (advances to next occurrence)
  - **Nth weekday**: every 2nd Friday, every 3rd Monday, etc. (advances to next month's occurrence)
...
curl -H "X-API-Key: your_secure_api_key_here" http://localhost:3001/
```
...
- `PORT`: Listening port (optional, defaults to 3001 locally, 8080 on Cloud Run).
...
# Includes: black, isort, autoflake, pylint, mypy, trivy
